...
class Bcs13thExamTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_13th_bcs_exam_is_seeded_with_100_questions(): void
    {
        $exam = Exam::where('slug', '13th-bcs-preliminary')->withCount('questions')->first();
        $this->assertNotNull($exam);
        $this->assertEquals(100, $exam->questions_count);
        $this->assertStringContainsString('১৩তম বিসিএস', $exam->title_bn);

        $questionsWithExplanations = Question::where('reference_source', '১৩তম বিসিএস প্রিলিমিনারি পরীক্ষা')
            ->whereNotNull('explanation_bn')
            ->count();
        $this->assertEquals(100, $questionsWithExplanations);
    }

    public function test_candidate_can_start_and_view_13th_bcs_exam(): void
    {
        $exam = Exam::where('slug', '13th-bcs-preliminary')->first();
        $this->assertNotNull($exam);

        $response = $this->actingAs($this->candidate)->get(route('exams.show', $exam->slug));
        $response->assertStatus(200);
        $response->assertSee('১৩তম বিসিএস');
        $response->assertSee('১০০');
    }

    public function test_candidate_can_submit_13th_bcs_attempt_and_view_explanations(): void
    {
        $exam = Exam::where('slug', '13th-bcs-preliminary')->first();
        $this->assertNotNull($exam);
        ...
        if ($q1->explanation_bn) {
            $resultResponse->assertSee('অকৃতজ্ঞতা');
        }
    }
}
