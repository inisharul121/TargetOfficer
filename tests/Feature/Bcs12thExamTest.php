...
class Bcs12thExamTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_12th_bcs_exam_is_seeded_with_100_questions(): void
    {
        $exam = Exam::where('slug', '12th-bcs-preliminary')->withCount('questions')->first();
        $this->assertNotNull($exam);
        $this->assertEquals(100, $exam->questions_count);
        $this->assertStringContainsString('১২তম বিসিএস (পুলিশ)', $exam->title_bn);

        $questionsWithExplanations = Question::where('reference_source', '১২তম বিসিএস (পুলিশ) প্রিলিমিনারি পরীক্ষা')
            ->whereNotNull('explanation_bn')
            ->count();
        $this->assertEquals(100, $questionsWithExplanations);
    }

    public function test_candidate_can_start_and_view_12th_bcs_exam(): void
    {
        $exam = Exam::where('slug', '12th-bcs-preliminary')->first();
        $this->assertNotNull($exam);

        $response = $this->actingAs($this->candidate)->get(route('exams.show', $exam->slug));
        $response->assertStatus(200);
        $response->assertSee('১২তম বিসিএস (পুলিশ)');
        $response->assertSee('১০০');
    }

    public function test_candidate_can_submit_12th_bcs_attempt_and_view_explanations(): void
    {
        $exam = Exam::where('slug', '12th-bcs-preliminary')->first();
        $this->assertNotNull($exam);

        // Enter exam room (creates attempt)
