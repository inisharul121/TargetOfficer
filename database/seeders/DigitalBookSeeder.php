...
class DigitalBookSeeder extends Seeder
{
    public function run(): void
    {
        $bcsType = ExamType::firstOrCreate(
            ...
            [
                'name_bn' => 'বিসিএস (প্রিলিমিনারি ও লিখিত)',
        ...
        if ($bangla) {
            $bookBangla = Book::updateOrCreate(
                ...
                [
                    ...
                    'title_bn' => 'বিসিএস বাংলা ভাষা ও সাহিত্য সমগ্র (MCQ + লিখিত)',
                    'title_en' => 'BCS Bengali Language & Literature Complete Volume',
                    'description_bn' => 'বিসিএস প্রিলিমিনারি ও লিখিত পরীক্ষার সম্পূর্ণ বাংলা সিলেবাস। প্রাচীন, মধ্য ও আধুনিক যুগের সাহিত্য, ব্যাকরণ, বাক্য শুদ্ধিকরণ এবং লিখিত অংশের রচনামূলক প্রশ্নোত্তর।',
            ...
            $chB1 = BookChapter::updateOrCreate(
                ...
                [
                    ...
                    'title_en' => 'Chapter 1: Bengali Phonetics, Orthography & Grammar',
                    'summary_bn' => 'ধ্বনি, বর্ণ, ণ-ত্ব ও ষ-ত্ব বিধান, সন্ধি, শব্দ ও বাংলা একাডেমি প্রমিত বানান নিয়মাবলী।',
                    'order' => 1,
                ]
            );

            // Attach Bangla questions to Chapter 1
            $bQuestions = Question::where('subject_id', $bangla->id)->where('status', 'published')->take(25)->pluck('id');
            $chB1->questions()->sync($bQuestions);

            // Written Materials for Chapter 1
            ...
            $bQuestions2 = Question::where('subject_id', $bangla->id)->where('status', 'published')->skip(25)->take(25)->pluck('id');
            $chB2->questions()->sync($bQuestions2);
        ...
        if ($english) {
            $bookEng = Book::updateOrCreate(
                ...
                [
                    ...
                    'title_bn' => 'বিসিএস ইংরেজি ভাষা ও সাহিত্য সমগ্র (MCQ + লিখিত)',
                    'title_en' => 'BCS English Language & Literature Complete Volume',
            ...
            $eQuestions = Question::where('subject_id', $english->id)->where('status', 'published')->take(25)->pluck('id');
            $chE1->questions()->sync($eQuestions);
            ...
            $eQuestions2 = Question::where('subject_id', $english->id)->where('status', 'published')->skip(25)->take(25)->pluck('id');
            $chE2->questions()->sync($eQuestions2);
        ...
        if ($bd) {
            $bookBD = Book::updateOrCreate(
                ...
                [
                    'exam_type_id' => $bcsType->id,
                    'subject_id' => $bd->id,
                    'title_bn' => 'বিসিএস বাংলাদেশ বিষয়াবলী সমগ্র (MCQ + লিখিত)',
                    'title_en' => 'BCS Bangladesh Affairs Complete Volume',
            ...
            $bdQuestions = Question::where('subject_id', $bd->id)->where('status', 'published')->take(25)->pluck('id');
            $chBD1->questions()->sync($bdQuestions);
            ...
            $bdQuestions2 = Question::where('subject_id', $bd->id)->where('status', 'published')->skip(25)->take(25)->pluck('id');
            $chBD2->questions()->sync($bdQuestions2);
        ...
        if ($intl) {
            $bookIntl = Book::updateOrCreate(
                ...
                [
                    'exam_type_id' => $bcsType->id,
                    'subject_id' => $intl->id,
                    'title_bn' => 'বিসিএস আন্তর্জাতিক বিষয়াবলী সমগ্র (MCQ + লিখিত)',
                    'title_en' => 'BCS International Affairs Complete Volume',
            ...
            $intlQuestions = Question::where('subject_id', $intl->id)->where('status', 'published')->take(25)->pluck('id');
            $chIntl1->questions()->sync($intlQuestions);
        ...
        if ($math) {
            $bookMath = Book::updateOrCreate(
                ...
                [
                    'exam_type_id' => $bcsType->id,
                    'subject_id' => $math->id,
                    'title_bn' => 'বিসিএস গাণিতিক যুক্তি ও মানসিক দক্ষতা সমগ্র (MCQ + লিখিত)',
                    'title_en' => 'BCS Mathematical Reasoning & Mental Ability',
                    'description_bn' => 'পাটিগণিত, বীজগণিত, জ্যামিতি ও ত্রিকোণমিতির প্রিলিমিনারি শর্টকাট এবং লিখিত পরীক্ষার পূর্ণাঙ্গ ধাপসহ সমাধান।',
            ...
            $chM1 = BookChapter::updateOrCreate(
                ...
                [
                    ...
                    'summary_bn' => 'লিখিত পরীক্ষার জন্য ধাপে ধাপে সমীকরণ সমাধান ও শর্টকাট টেকনিক।',
                    'order' => 1,
                ]
            );

            $mQuestions = Question::where('subject_id', $math->id)->where('status', 'published')->take(25)->pluck('id');
            $chM1->questions()->sync($mQuestions);
        ...
        if ($science) {
            $bookSci = Book::updateOrCreate(
                ...
                [
                    ...
                    'title_bn' => 'বিসিএস সাধারণ বিজ্ঞান ও তথ্যপ্রযুক্তি সমগ্র (MCQ + লিখিত)',
                    'title_en' => 'BCS General Science & ICT Complete Volume',
            ...
            $sQuestions = Question::where('subject_id', $science->id)->where('status', 'published')->take(25)->pluck('id');
            $chS1->questions()->sync($sQuestions);
